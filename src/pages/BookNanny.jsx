  ...
  return (
    <div className="max-w-lg mx-auto pb-12">
      ...
      <div className="bg-card border border-border/40 rounded-3xl p-7 shadow-sm space-y-6">
          ...
          <div className="space-y-4">
            ...
            {hours > 0 && (
              <div className="inline-flex items-center gap-2 bg-sage/15 text-sage-foreground text-sm px-3.5 py-2 rounded-xl font-medium">
                <Clock className="w-3.5 h-3.5" /> {hours.toFixed(1)} sati
              </div>
            )}
          </div>
        </div>
        ...
        {/* Booking type */}
        <div>
          <Label className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-3 block">Vrsta rezervacije</Label>
          <RadioGroup value={form.booking_type} onValueChange={v => update('booking_type', v)} className="flex gap-3">
            {[
              { value: 'one_time', label: 'Jednokratno', sub: 'Jedan termin' },
              { value: 'recurring', label: 'Ponavljajuće', sub: 'Tjedni raspored' },
            ].map(opt => (
        ...
        <div className="space-y-4">
          <div>
            <Label className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-1.5 block">Imena djece</Label>
            <Input value={form.children_names} onChange={e => update('children_names', e.target.value)} placeholder="npr. Marko i Ana" className="rounded-xl" />
          </div>
          <div>
            <Label className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-1.5 block">Posebne upute</Label>
            <Textarea value={form.notes} onChange={e => update('notes', e.target.value)} placeholder="Alergije, raspored spavanja, kućna pravila…" rows={3} className="rounded-xl resize-none" />
          </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-ivory to-rose-light/30 rounded-2xl p-5 space-y-3">
          ...
          <div className="flex justify-between text-sm">
            <span className="text-muted-foreground">{hours > 0 ? `${hours.toFixed(1)} h` : '0 h'} × €{rate}/h</span>
        ...
        >
          {bookMutation.isPending ? (
            ...
            <span className="flex items-center gap-2"><CheckCircle2 className="w-4 h-4" /> Pošalji zahtjev · €{totalCost.toFixed(2)}</span>
          ) : (
